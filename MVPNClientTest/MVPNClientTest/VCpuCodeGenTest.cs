...
namespace SoftEther.WebSocket.Helper
{
    ...
    class VCodeOperand
    {
        ...
        public string GenerateMemoryAccessCode(uint codeAddress, bool writeMode, string srcOrDestinationCode)
        {
            ...
            if (writeMode)
            {
                ...
                w.WriteLine($"    if (mem_write(0x{codeAddress:x}, {GetCode()}, write_tmp) == false) goto L_RETURN;"); 
            ...
            {
                w.WriteLine($"    if (mem_read(0x{codeAddress:x}, {GetCode()}, &read_tmp) == false) goto L_RETURN;");
    ...
    class VCodeGen
    {
        ...
        void WriteMainFunction()
        {
            ...
            Out.WriteLine("uint write_tmp = 0;");
            Out.WriteLine("uint read_tmp = 0;");
...
// Write
bool mem_write(uint code_ip, uint vaddr, uint value)
{
    uint vaddr1_index = vaddr / VConsts.PageSize;
    uint vaddr1_offset = vaddr % VConsts.PageSize;
    if (vaddr1_index == cache_last_page1)
    {
        *((uint*)(((byte*)cache_last_realaddr1) + vaddr1_offset)) = value;
    }
    else if (vaddr1_index == cache_last_page2)
    {
        *((uint*)(((byte*)cache_last_realaddr2) + vaddr1_offset)) = value;
    }
    else
    {
        if (pte[vaddr1_index].CanWrite == false)
...
bool mem_read(uint code_ip, uint vaddr, uint* value)
{
    uint vaddr1_index = vaddr / VConsts.PageSize;
    uint vaddr1_offset = vaddr % VConsts.PageSize;
    if (vaddr1_index == cache_last_page1)
    {
        *value = *((uint*)(((byte*)cache_last_realaddr1) + vaddr1_offset));
    }
    else if (vaddr1_index == cache_last_page2)
    {
        *value = *((uint*)(((byte*)cache_last_realaddr2) + vaddr1_offset));
    }
    else
    {
        if (pte[vaddr1_index].CanRead == false)
